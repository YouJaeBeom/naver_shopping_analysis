import topKproducts_avgPrice
import topKproducts_avgpurCnt

if __name__ == "__main__" :
    keyword="애플워치"
    top_10 = topKproducts_avgPrice.avg_products_price(keyword,10,1)
    top_10_avg_price = sum(top_10)/len(top_10)
    print("sum(top_10) :", sum(top_10), "len(top_10 : ", len(top_10) , "avg_price : ", top_10_avg_price )

    top_40 = topKproducts_avgPrice.avg_products_price(keyword,40,1)
    top_40_avg_price = sum(top_40)/len(top_40)
    print("sum(top_40) :", sum(top_10), "len(top_40 : ", len(top_40) , "avg_price : ", top_40_avg_price )

    keyword="애플워치"
    top_10 = topKproducts_avgpurCnt.avg_products_purchaseCnt(keyword,10,1)
    ...
    top_40 = topKproducts_avgpurCnt.avg_products_purchaseCnt(keyword,40,1)
